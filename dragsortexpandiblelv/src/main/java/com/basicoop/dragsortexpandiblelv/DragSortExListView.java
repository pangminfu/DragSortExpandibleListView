...
package com.basicoop.dragsortexpandiblelv;
 ...
 *
 */
public class DragSortExListView extends ExpandableListView {
    ...
    public DragSortExListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs,
                    R.styleable.DragSortExListView, 0, 0);

            mItemHeightCollapsed = Math.max(1, a.getDimensionPixelSize(
                    R.styleable.DragSortExListView_collapsed_height, 1));

            mTrackDragSort = a.getBoolean(
                    R.styleable.DragSortExListView_track_drag_sort, false);
            ...
            mFloatAlpha = a.getFloat(R.styleable.DragSortExListView_float_alpha, mFloatAlpha);
            mCurrFloatAlpha = mFloatAlpha;

            mDragEnabled = a.getBoolean(R.styleable.DragSortExListView_drag_enabled, mDragEnabled);

            mSlideRegionFrac = Math.max(0.0f,
                    Math.min(1.0f, 1.0f - a.getFloat(
                            R.styleable.DragSortExListView_slide_shuffle_speed,
                            0.75f)));

            mAnimate = mSlideRegionFrac > 0.0f;

            float frac = a.getFloat(
                    R.styleable.DragSortExListView_drag_scroll_start,
                    mDragUpScrollStartFrac);

            setDragScrollStart(frac);

            mMaxScrollSpeed = a.getFloat(
                    R.styleable.DragSortExListView_max_drag_scroll_speed,
                    mMaxScrollSpeed);

            removeAnimDuration = a.getInt(
                    R.styleable.DragSortExListView_remove_animation_duration,
                    removeAnimDuration);

            dropAnimDuration = a.getInt(
                    R.styleable.DragSortExListView_drop_animation_duration,
                    dropAnimDuration);

            boolean useDefault = a.getBoolean(
                    R.styleable.DragSortExListView_use_default_controller,
                    true);

            if (useDefault) {
                boolean removeEnabled = a.getBoolean(
                        R.styleable.DragSortExListView_remove_enabled,
                        false);
                int removeMode = a.getInt(
                        R.styleable.DragSortExListView_remove_mode,
                        DragSortController.FLING_REMOVE);
                boolean sortEnabled = a.getBoolean(
                        R.styleable.DragSortExListView_sort_enabled,
                        true);
                int dragInitMode = a.getInt(
                        R.styleable.DragSortExListView_drag_start_mode,
                        DragSortController.ON_DOWN);
                int dragHandleId = a.getResourceId(
                        R.styleable.DragSortExListView_drag_handle_id,
                        0);
                int flingHandleId = a.getResourceId(
                        R.styleable.DragSortExListView_fling_handle_id,
                        0);
                int clickRemoveId = a.getResourceId(
                        R.styleable.DragSortExListView_click_remove_id,
                        0);
                int bgColor = a.getColor(
                        R.styleable.DragSortExListView_float_background_color,
                        Color.BLACK);
     ...
     */
    public ExpandableListAdapter getInputAdapter() {
    ...
    private class AdapterWrapper extends BaseExpandableListAdapter {
        private ExpandableListAdapter mAdapter;

        public AdapterWrapper(ExpandableListAdapter adapter) {
            super();
            mAdapter = adapter;
...
//        }
		...
		public View getGroupView(int groupPosition, boolean isExpanded,
            ...
            if (convertView != null) {
                ...
                child = mAdapter.getGroupView(groupPosition, false, oldChild, DragSortExListView.this);
                if (child != oldChild) {
                    // shouldn't get here if user is reusing convertViews
                    // properly
                    if (oldChild != null) {
                ...
                child = mAdapter.getGroupView(groupPosition, false, null, DragSortExListView.this);
                if (child instanceof Checkable) {
                    v = new DragSortItemViewCheckable(getContext());
                } else {
                    v = new DragSortItemView(getContext());
    ...
    private boolean updatePositions() {
        ...
        if (mFloatViewMid < edge) {
            ...
            // Log.d("mobeta", "    edge="+edge);
            final int count = getInputAdapter().getGroupCount();
            while (itemPos < count) {
                if (itemPos == count - 1) {
                    edge = itemTop + divHeight + itemHeight;
                    break;
     ...
     * through to {@link com.sph.dragableexlistview.DragSortListView.DropListener#drop(int, int)}.
     * 
        ...
        if (v != null) {
            ...
            if (type >= 0) {
                if (mSampleViewTypes[type] == null) {
                    v = adapter.getGroupView(position, false, null, this);
                    mSampleViewTypes[type] = v;
                } else {
                    v = adapter.getGroupView(position, false, mSampleViewTypes[type], this);
                }
            } else {
    ...
    public interface FloatViewManager {
         ...
         * and the offset provided in {@link com.sph.dragableexlistview.DragSortListView#startDrag}.
     ...
     * {@link com.sph.dragableexlistview.DragSortListView.DragScrollProfile}.
     * 
     * @param ssp
     */
     ...
     * {@link com.sph.dragableexlistview.DragSortListView#setDragScrollProfile(com.sph.dragableexlistview.DragSortListView.DragScrollProfile)} to
     * set custom profile.
     * 
     * @author heycosmo
     *
    ...
    private class DragScroller implements Runnable {
        ...
        public void stopScrolling(boolean now) {
            if (now) {
                DragSortExListView.this.removeCallbacks(this);
                mScrolling = false;
            } else {
                mAbort = true;
            }
        ...
        public void run() {
            ...
            DragSortExListView.this.layoutChildren();
            invalidate();
